...
File compliance with these archive specifications can and should be checked using the IOOS Compliance Checker, utilizing both its built-in plugin `cf:1.11` and the dedicated [WCRP plugin](https://github.com/ESGF/cc-plugin-wcrp) `wcrp_cordex_cmip6`.
For convenience, the [esgf-qa](https://esgf.github.io/esgf-qa) tool orchestrates the application of the mentioned compliance checker plugins across all files of a simulation while conducting additional inter-file and inter-dataset consistency and continuity checks.
Data files raising high severity errors in these checks will not be published on ESGF.
...
The next sections show some formatting examples as NetCDF Common Data Language ([CDL](https://docs.unidata.ucar.edu/nug/2.0-draft/cdl.html)) snippets. Full examples can be found at [https://wcrp-cordex.github.io/cordex-cmip6-cmor-tables/cmor-examples.html](https://wcrp-cordex.github.io/cordex-cmip6-cmor-tables/cmor-examples.html).
...
// global attributes:
  ...
  :frequency = "mon" ;
  :grid = "Lambert conic conformal with 25 km grid spacing"
  :history = "2026-09-07T13:08:51Z ;rewrote data to be consistent with CORDEX-CMIP6 for variable tas found in table mon."
  ...
  :source_id = "InterRCM1" ;
  :source_type = "ARCM" ;
  :title = "IIRCM InterRCM1 downscaling of ERA5 evaluation for CORDEX-CMIP6 AFR-25"
